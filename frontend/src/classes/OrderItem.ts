...
export class OrderItem {
    ...
    async RefreshReadyNumber(){
        ...
        await axios.get(`${import.meta.env.VITE_APP_BACKEND_HOST}${import.meta.env.VITE_APP_BACKEND_VERSION}/api/products/${this.Id}`,
            {
                headers: {
    ...
    async RefreshProductData(){
        await axios.get(`${import.meta.env.VITE_APP_BACKEND_HOST}${import.meta.env.VITE_APP_BACKEND_VERSION}/api/products/${this.product.id}/recipetree`,
            {
                headers: {
                    'Authorization': `Bearer ${zitadelAuth.oidcAuth.accessToken}`
                }
    ...
    async UpdateMaterialEntryCost(materialIndex: number){
        if (this.materials[materialIndex].entry != undefined){
            await axios.get(`${import.meta.env.VITE_APP_BACKEND_HOST}${import.meta.env.VITE_APP_BACKEND_VERSION}/api/materials/${this.materials[materialIndex].material.id}/entries/${this.materials[materialIndex].entry.id}/cost?quantity=${this.materials[materialIndex].quantity}`, {
                headers: {
                    'Authorization': `Bearer ${zitadelAuth.oidcAuth.accessToken}`
                }
            }).then((response) => {
